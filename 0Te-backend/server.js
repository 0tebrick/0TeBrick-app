...
if (process.env.CORS_ORIGIN){
  ...
  try {
    ...
    console.error(`Error fetching minifigs for set ${setID}:`, error.message); 
    if (error.bricklinkDetails) {
            console.error("BrickLink API detailed error:", error.bricklinkDetails);
        }
    const statusCode = error.statusCode || 500;
    let message = error.message || "Error al obtener minifigs desde Bricklink";
    if (error.bricklinkDetails && error.bricklinkDetails.rawHtmlError) {
            message = `Error de BrickLink (HTML): ${error.message}`;
        }
